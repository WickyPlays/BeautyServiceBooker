...
import { styles } from "./FavoritesBookingItem.style";
import { STATUS_STYLES } from "../../../commons/common_style";

const FavoritesBookingItem = ({ bookings, handleFavoriteToggle, handleServiceItemPress }) => {
  if (!bookings || bookings.length === 0) {
    return (
      <View style={styles.noBookingsContainer}>
        <Text style={styles.noBookingsText}>No favorite bookings found.</Text>
      </View>
    );
  }
    ...
    <>
      {bookings.map((booking) => (
        <View key={booking.id} style={styles.bookingItem}>
          ...
          <View style={styles.buttonRow_Favorites}>
            <TouchableOpacity 
              style={booking.status === 'pending' ? styles.upcomingButton : styles.pastButton}
            >
              <Text style={styles.buttonText}>
                {booking.status === 'pending' ? 'Upcoming Booking' : 'Past Booking'}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
...
export default FavoritesBookingItem;
