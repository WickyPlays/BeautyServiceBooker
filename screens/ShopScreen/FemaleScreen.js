...
export default function FemaleScreen({ navigation }) {
  ...
  const salonData = {
    ...
    banner: 'https://images.pexels.com/photos/3992863/pexels-photo-3992863.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
    rating: 4.1,
    ratingCount: '5k+',
    distance: '5.0 Kms',
    priceLevel: '$$',
  ...
  const fetchServices = async () => {
    try {
      ...
      if (Array.isArray(response.data)) {
        const filteredServices = response.data.filter(service => service.gender === 'female' || service.gender === 'both');
